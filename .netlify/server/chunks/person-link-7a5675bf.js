  ...
  return `${person ? `<a${(0, import_index_277e1cdb.a)("href", "/people/" + (0, import_get.default)(person, "slug.current"), 0)} sveltekit:prefetch class="${"person-link svelte-j84dxt"}"${(0, import_index_277e1cdb.a)("style", tiny ? "font-size: 11px" : "", 0)}${(0, import_index_277e1cdb.a)("this", linkEl, 0)}>${person.title.length < 20 ? `<!-- HTML_TAG_START -->${overrideText ? (0, import_global_61b718ff.g)(overrideText) : person.title}<!-- HTML_TAG_END -->` : `<!-- HTML_TAG_START -->${overrideText ? (0, import_global_61b718ff.g)(overrideText) : person.title.slice(0, 20) + "..."}<!-- HTML_TAG_END -->`}</a>
	...
	<div class="${"pop-up svelte-j84dxt"}" aria-expanded="${"false"}"${(0, import_index_277e1cdb.a)("this", popEl, 0)}>
		<div class="${"column first svelte-j84dxt"}">
			...
			<div class="${"badges"}">${person.role ? `<div class="${"badge"}">${(0, import_index_277e1cdb.e)(import_global_61b718ff.r[person.role])}</div>` : ``}
    ...
    return `<div class="${"badge"}">${(0, import_index_277e1cdb.e)(tag.label)}</div>`;
		...
		<div class="${"column second"}">
			${person.mainImage ? `<img class="${"image"}"${(0, import_index_277e1cdb.a)("alt", person.title, 0)}${(0, import_index_277e1cdb.a)("src", (0, import_sanity_59332496.u)(person.mainImage).quality(90).saturation(-100).width(100).height(100).url(), 0)}>` : ``}</div></div>` : ``}`;
});
